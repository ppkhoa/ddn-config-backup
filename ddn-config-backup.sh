...
# GRIDScaler 4.x upgrade/reinstall or EXAScaler 3.x/4.x upgrade/reinstall.
...
# $2 - doc target dir
    ...
    cp -r --parents /etc/sysconfig/network-scripts/ifcfg-* "$backup_dir"
        for file in /etc/sysconfig/network-scripts/route-*
    do
        if [ -e "$file" ]
        then
...
function gs_restore {
	...
	if $(echo $restore_file | if grep -q gs; then true; else false; fi)
		then
...
function sshkeyscan {
	...
	grep -E -v '^\s*#' /etc/hosts | sed -e 's/#.*//' | awk '{for(i=1;i<=NF;i++) print $i }' | sort | uniq > sshkeyhosts.txt # For every line in the hosts file, get IP address and all hostname, sort and filter duplicates
